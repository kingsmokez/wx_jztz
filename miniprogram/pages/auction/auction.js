...
Page({
  ...
  loadList: function() {
    ...
    api.callFunctionWithTimeout('auctionPicker', 'list', {}, 60000, { success: true, data: [], cached: false }).then(function(res) {
      var now = new Date()
      that.setData({
        stocks: (res && res.data) || [],
  ...
  refresh: function() {
    var that = this
    that.setData({ loading: true })
    api.callFunctionWithLoading('auctionPicker', 'run', { topN: 20, force: true }).then(function(res) {
      var now = new Date()
