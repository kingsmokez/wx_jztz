...
function simulatePickV43b(dayQuotes, klineMap, dateIdxMap, topN, marketEnv, ma5Min, ma10Min, rsiMax, adxMin, v31W, v10W, minScoreOverride, filters) {
  ...
  var minScore = (filters && filters.minScore !== undefined) ? filters.minScore : (minScoreOverride !== undefined ? minScoreOverride : (params._minScore || 55))
  var scored = []
  ...
  for (var i = 0; i < dayQuotes.length; i++) {
    ...
    var pp = calcPricePositionVsHigh(klines, dateIdx); if (filters && filters.pricePosThreshold !== undefined) { if (pp < filters.pricePosThreshold) continue } else if (params.pricePosFilter) { if (pp < (params.pricePosThreshold || 0.75)) continue }
    if (filters && filters.vrMin !== undefined) { if (volumeRatio < filters.vrMin) continue } else if (params.volumeRatioFilter && volumeRatio < (params.volumeRatioMin || 1.5)) continue
    var rs = calcRelativeStrength(klines, dateIdx); if (filters && filters.rsMin !== undefined) { if (rs < filters.rsMin) continue } else if (params.rsFilter) { if (rs < (params.rsThreshold || 0)) continue }
    var morphBonus = 0
...
async function runBacktest() {
  ...
  console.log('\nResults saved to backtest/results/backtest_v61.txt')
}
