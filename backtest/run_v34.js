// run_v34.js - V34策略回测: 自适应市场+软过滤+动态连涨+形态加分
var fs = require("fs")
...
// V34策略: 自适应市场+软过滤+动态连涨+形态加分
function simulatePickV34(dayQuotes, klineMap, dateIdxMap, topN, params, marketEnv, minScore) {
  var scored = []
  var adxThreshold = params.adxThreshold || 20
  var bollThreshold = params.bollThreshold || 0.85
  var maxConsecUp = params.maxConsecUp || 99

  if (params.adaptiveMarket && marketEnv) {
    if (marketEnv.trend === "bear") {
      adxThreshold = Math.max(20, adxThreshold + (params.bearAdxBump || 5))
      bollThreshold = Math.max(0.75, bollThreshold - (params.bearBollDrop || 0.1))
...
var VARIANTS = {
  "V32b_ref":    { v31Weight: 0.75, v10Weight: 0.25 },
  "V33e_ref":    { v31Weight: 0.75, v10Weight: 0.25, adaptiveMarket: true },
  "V33f_ref":    { v31Weight: 0.75, v10Weight: 0.25, filterConsecUp: true, maxConsecUp: 5, morphBonus: true },
  "V34a_adp_morph": { v31Weight: 0.75, v10Weight: 0.25, adaptiveMarket: true, morphBonus: true },
  "V34b_soft_vol":  { v31Weight: 0.75, v10Weight: 0.25, adaptiveMarket: true, morphBonus: true, softVolConfirm: true, volPenalty: 0.9 },
  "V34c_dyn_consec":{ v31Weight: 0.75, v10Weight: 0.25, adaptiveMarket: true, morphBonus: true, softVolConfirm: true, volPenalty: 0.9, dynamicConsec: true, consecBear: 3, consecNeutral: 5, consecBull: 6 },
  "V34d_80_20":   { v31Weight: 0.8, v10Weight: 0.2, adaptiveMarket: true, morphBonus: true, softVolConfirm: true, volPenalty: 0.9, dynamicConsec: true, consecBear: 3, consecNeutral: 5, consecBull: 6 },
  "V34e_min60":   { v31Weight: 0.75, v10Weight: 0.25, adaptiveMarket: true, morphBonus: true, softVolConfirm: true, volPenalty: 0.9, dynamicConsec: true, consecBear: 3, consecNeutral: 5, consecBull: 6, _minScore: 60 },
  "V34f_strict_adx":{ v31Weight: 0.75, v10Weight: 0.25, adaptiveMarket: true, morphBonus: true, softVolConfirm: true, volPenalty: 0.9, dynamicConsec: true, consecBear: 3, consecNeutral: 5, consecBull: 6, adxThreshold: 22 },
  "V34g_vol_085":  { v31Weight: 0.75, v10Weight: 0.25, adaptiveMarket: true, morphBonus: true, softVolConfirm: true, volPenalty: 0.85, dynamicConsec: true, consecBear: 3, consecNeutral: 5, consecBull: 6 },
  "V34h_bear_strict":{ v31Weight: 0.75, v10Weight: 0.25, adaptiveMarket: true, morphBonus: true, softVolConfirm: true, volPenalty: 0.85, dynamicConsec: true, consecBear: 3, consecNeutral: 5, consecBull: 6, bearAdxBump: 8, bearBollDrop: 0.15 },
}

async function runBacktest() {
  console.log("=".repeat(60))
  console.log("V34 Strategy: Adaptive + Soft-filters + Dynamic Consec")
  console.log("=".repeat(60))
  ...
  for (var si = 0; si < sampleDates.length; si++) {
    ...
    for (var i = 0; i < dayQuotes.length; i++) {
      ...
      if (score >= CONFIG.minScore) {
        var returns = calcHoldingReturn(stock.price, klines, dateIdx, CONFIG.holdDays)
        if (returns) v10Picks.push({ date: dateStr, code: stock.code, price: stock.price, returns: returns })
      }
    }
    ...
    for (var v = 0; v < variantNames.length; v++) {
      ...
      var minScore = VARIANTS[vName]._minScore || CONFIG.minScore
      var picks = simulatePickV34(dayQuotes, klineMap, dateIdxMap, CONFIG.topN, VARIANTS[vName], marketEnv, minScore)
  ...
  report.push("V34 Strategy Backtest - Adaptive + Soft-filters + Dynamic Consec")
  report.push("=".repeat(80))
  ...
  report.push("SUMMARY")
  report.push("=".repeat(80))
  report.push("V10: 3dWR=" + v10Stats.hold3.winRate + "% 3dAR=" + v10Stats.hold3.avgReturn + "% 5dWR=" + v10Stats.hold5.winRate + "% 5dAR=" + v10Stats.hold5.avgReturn + "% 7dWR=" + v10Stats.hold7.winRate + "% 7dAR=" + v10Stats.hold7.avgReturn + "% 10dWR=" + v10Stats.hold10.winRate + "% 10dAR=" + v10Stats.hold10.avgReturn + "%")
  report.push("")
  report.push("Variant                   n    3dWR  3dAR  5dWR  5dAR  7dWR  7dAR 10dWR 10dAR  BeatV10")
  ...
  for (var v = 0; v < variantNames.length; v++) {
    ...
    var line = vName.padEnd(26) + String(picks.length).padStart(6)
    var beatsAll = true
    var beatsCount = 0
    for (var h = 0; h < CONFIG.holdDays.length; h++) {
      var d = CONFIG.holdDays[h]
  ...
  console.log("\n" + reportText)
  fs.writeFileSync(path.join(CONFIG.outputDir, "backtest_v34.txt"), reportText, "utf8")
  console.log("\nReport saved to backtest/results/backtest_v34.txt")
}
