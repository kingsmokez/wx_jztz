...
function simulatePickV43b(dayQuotes, klineMap, dateIdxMap, topN, marketEnv) {
  ...
  for (var i = 0; i < dayQuotes.length; i++) {
    ...
    // V49e: MA5斜率>0.1 + MA10斜率>0 硬过滤
    if (techData.ma5Slope !== undefined && techData.ma5Slope < 0.1) continue
    if (techData.ma10Slope !== undefined && techData.ma10Slope < 0) continue
...
var EXIT_STRATEGIES = {
  // V49e: V48b退出策略
  "V49e_MA5_0.1_MA10_0": { stopLoss: -100, trailingRules: [
...
async function runBacktest() {
  ...
  fs.writeFileSync(path.join(CONFIG.outputDir, 'backtest_v49e.txt'), output.join('\n'), 'utf8')
