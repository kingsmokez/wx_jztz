// run_v78.js - V78策略回测: MACD金叉确认深度优化
// V76发现: MACD金叉确认最有效(WR=86.49% AR=4.31%)但n=37太少
// V78目标: 找到WR>90%且AR>3.08%的策略(同时超越V73)
...
function calcPatternScoreV78(stock, klines, dateIdx, tech, volumeRatio, bollWidthMax) {
...
function simulatePickV78(dayQuotes, klineMap, dateIdxMap, topN, marketEnv, strategy) {
  ...
  for (var i = 0; i < dayQuotes.length; i++) {
    ...
    var patternResult = calcPatternScoreV78(stock, klines, dateIdx, tech, volumeRatio, strategy.bollWidthMax)
...
async function runBacktest() {
  ...
  console.log('V78 Strategy Backtest: MACD金叉确认深度优化')
  console.log('='.repeat(80))
  ...
  var strategyNames = Object.keys(V78_STRATEGIES)
  var dynamicPicks = {}
  ...
  for (var si = 0; si < sampleDates.length; si++) {
    ...
    for (var s = 0; s < strategyNames.length; s++) {
      ...
      var strategy = V78_STRATEGIES[sName]
      var picks = simulatePickV78(dayQuotes, klineMap, dateIdxMap, CONFIG.topN, marketEnv, strategy)
  ...
  fs.writeFileSync(path.join(CONFIG.outputDir, 'backtest_v78.txt'), output.join('\n'), 'utf8')
  console.log('\nResults saved to backtest/results/backtest_v78.txt')
